...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    const profileRole = role === 'admin' ? 'admin' : 'teacher';
    ...
    const { data: authData, error: authError } = await supabase.auth.admin.createUser({
      ...
      password,
      email_confirm: true,
      user_metadata: { displayName, role: profileRole, subject },
    });
    ...
    const { data, error } = await supabase
      .from('users')
      .insert({ uid, email, displayName, role: profileRole, subject: subject || null })
